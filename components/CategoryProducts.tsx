...
import { SlidersHorizontal, X, ChevronDown, Search, Filter } from "lucide-react";

import { Category } from "@/sanity.types"; // Ensure this exists or use any[]
import { client } from "@/sanity/lib/client";
import ProductCard from "./ProductCard"; // Ensure this component is updated too
...
interface Props {
  categories: Category[];
  slug: string;
  materials: any[]; // In your pantry context, treat these as "Dietary Tags" (e.g. Vegan, Gluten Free)
}
...
const CategoryProducts = ({ categories, slug, materials }: Props) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // 1. Base Filters
        let filters = [`_type == 'product'`]; // Removed isHidden check for simplicity unless you use it
        ...
        if (selectedMaterials.length > 0) {
          // Assuming 'materials' field exists in schema. If you changed it to 'benefits', update here.
        ...
        if (sortOption === "new-arrivals") orderClause = "order(_createdAt desc)";
        else if (sortOption === "price-low") orderClause = "order(price asc)";
        else if (sortOption === "price-high") orderClause = "order(price desc)";
        ...
        const query = `
          *[${filters.join(" && ")}] | ${orderClause} {
            ...
            badge,     // New field
            subtitle,  // New field
            mainImage, // New field (Single image)
            images,    // Fallback if multiple
  ...
  }, [currentSlug, selectedMaterials, priceRange, sortOption]);
  ...
  return (
    <div className="pb-20 bg-cream min-h-screen">
      ...
      {/* 1. CATEGORY STRIP (Pills) */}
      <div className="sticky top-0 z-40 bg-cream/95 backdrop-blur-md border-b border-charcoal/5 pt-4 pb-4">
                ...
                <button 
                   ...
                   className={`px-6 py-2 rounded-full text-sm font-bold tracking-wide transition-all border ${
                     currentSlug === "all" 
                       ...
                       : "bg-white text-charcoal/60 border-charcoal/10 hover:border-charcoal/40"
                   }`}
                >
                   All Items
                </button>
             ...
             <div className="flex flex-wrap items-center justify-between gap-3 bg-white rounded-2xl border border-charcoal/5 p-3 shadow-sm">
                ...
                <Sheet>
                    ...
                    <SheetContent side="right" className="w-[340px] flex flex-col h-full bg-cream border-l border-charcoal/10">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    {materials?.map((item: any) => {
                                        ...
                                        return (
                                            <div key={item._id} className="flex items-center justify-between cursor-pointer group p-2 hover:bg-white rounded-lg transition-colors" onClick={() => item.slug && handleMaterialToggle(item.slug)}>
                                                <div className="flex items-center gap-3">
                                                    <Checkbox 
                                                        checked={selectedMaterials.includes(item.slug)} 
                                                        onCheckedChange={()=>{}} 
                                                ...
                                                <span className="text-xs text-charcoal/30 font-bold">{materialCounts[item.slug] || 0}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                        ...
                        <SheetFooter className="border-t border-charcoal/5 pt-4 gap-3 sm:gap-0">
                             <Button variant="outline" onClick={clearFilters} className="flex-1 rounded-full border-charcoal/10 text-charcoal/60 hover:text-charcoal">Clear</Button>
                             <SheetClose asChild>
                ...
                <Select value={sortOption} onValueChange={setSortOption}>
                    ...
                    <SelectContent align="end" className="bg-white border-charcoal/10 rounded-xl shadow-lg">
                        <SelectItem value="new-arrivals" className="focus:bg-cream cursor-pointer">Newest Arrivals</SelectItem>
                        <SelectItem value="price-low" className="focus:bg-cream cursor-pointer">Price: Low - High</SelectItem>
                        <SelectItem value="price-high" className="focus:bg-cream cursor-pointer">Price: High - Low</SelectItem>
                    </SelectContent>
                </Select>
             </div>
             
             {/* Active Filter Chips */}
             {(selectedMaterials.length > 0 || priceRange.min > 0) && (
                <div className="flex flex-wrap gap-2 mt-3 px-1">
                   {selectedMaterials.map(m => (
                       <span key={m} className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-white border border-charcoal/10 flex items-center gap-2 text-charcoal">
                           {m} <X size={12} className="cursor-pointer hover:text-brandRed" onClick={()=>handleMaterialToggle(m)}/>
                       </span>
                   ))}
                   {priceRange.min > 0 && (
                       <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-white border border-charcoal/10 flex items-center gap-2 text-charcoal">
                           LKR {priceRange.min} - {priceRange.max} <X size={12} className="cursor-pointer hover:text-brandRed" onClick={()=>setPriceRange({min:0, max:20000})}/>
                       </span>
                   )}
                </div>
             )}
